...
const desencriptar = txt => {
  const msjDesencriptado = txt.replace(/ai|enter|imes|ober|ufat/g, msjDesencriptar);
  return msjDesencriptado;
};
btnEncriptar.addEventListener("click", event => {
  event.preventDefault();
  const txt = msj.value;
  msjFinal.value = ""; // Limpia el textarea resultante antes de asignar el nuevo valor encriptado
...
btnDesencriptar.addEventListener("click", event => {
  event.preventDefault();
  const txt = msj.value;
  msjFinal.value = ""; // Limpia el textarea resultante antes de asignar el nuevo valor desencriptado
